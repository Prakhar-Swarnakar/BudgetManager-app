...
| R23 | Changing the database after real data exists | A wrong migration can corrupt or lose history | Medium | Use Room, version the schema, and test migrations with sample data (see [12-tech-architecture.md](12-tech-architecture.md)). First real migration (v1→v2, adding sms_message.payment_method, 2026-09-25): Room's own `MigrationTestHelper` couldn't be used - it hit an `AbstractMethodError`, a genuine incompatibility between Room 2.8.5's precompiled `room-testing` serializer classes and the `kotlinx-serialization-core` version Room itself strictly pins, not fixable from this app's build file. Verified instead by hand-diffing the exported v1/v2 schema JSONs and by installing over the real on-device v1 database and confirming the column and every existing row survived. Revisit MigrationTestHelper if a later Room release fixes it | 2 |
