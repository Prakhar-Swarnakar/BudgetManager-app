...
| M5 | Built, not yet tested on-device | 2026-09-25 | Categories page: a plain-Column list (drag handle to reorder, swipe left to archive), a collapsed Archived section that expands and restores on tap, and a bottom sheet shared between "new category" and "rename/change icon" (EmojiIconBox + name). The + button lives in the top bar via a new `actions` slot on AppScaffold. Reordering is local-only until drag end, then persisted in one batch write (CategoryRepository.reorder, CategoryDao.updateAll). Single-emoji validation (EmojiValidation, pure Kotlin, grapheme-cluster based) has tests for skin tones, ZWJ families, and flags. CategoriesViewModel has a full unit test suite against the fake repository. Installed on-device but not yet exercised by hand - the drag-handle-plus-swipe-to-archive combination on one row (R30 in [09-risks-and-phases.md](09-risks-and-phases.md)) is the part most worth testing first |
