...
| M2 | Done - 2a and 2c on 2026-09-25, 2b on 2026-09-25 | 2026-09-25 | 2a/2c: real SmsReceiver (via a manual Hilt @EntryPoint, not @AndroidEntryPoint - BroadcastReceiver.onReceive() is abstract in the Android SDK and can never call super, a genuine Dagger limitation, not a workaround choice), PermissionGate, Notifier, InboxScanner catch-up scan. Verified end-to-end against a real ICICI Bank SMS, not just test strings: classified, stored, notified, checked directly in the database. Fixed a first-run bug before it could bulk-import the phone's SMS history (out of scope per 06-backlog.md). 2b: built without the planned 30-50-sample collection - instead analysed a week of the project's own real SMS directly (SmsParser + BankRules: 3 verified formats - ICICI UPI-to-person, ICICI autopay, SBI Credit Card - plus a generic fallback for other banks; CategorySuggester wired to the seeded keyword list). Found and fixed a real classifier bug in the same data: "will be debited" autopay/due-date pre-notices were being counted as completed spends, creating a false duplicate next to the real debit SMS that follows. Added sms_message.payment_method (schema v1→v2, MIGRATION_1_2), verified by hand-diffing the exported schema JSONs and by installing over the real on-device v1 database and confirming the column and every existing row survived - MigrationTestHelper itself hit an unrelated upstream Room 2.8.5 bug (see 09-risks-and-phases.md R23) |
...
| M7 | Built, not yet tested on-device | 2026-09-25 | Home: month selector (past viewable, future disabled - the restriction Monthly budget's doesn't have), a summary card (spent of budget, amount left, progress bar), a "N messages to review" line, a card per active category (own progress bar and status word, same order as Monthly budget's list), and a round Add FAB for a manual transaction. Category detail: month selector, a summary card, and that category's transactions (newest first), tap to edit, swipe left to delete - deleting reverts a linked message to Not assigned. Originally shipped with an Undo bar, replaced the same day with a `ConfirmDialog` ("Delete transaction? This can't be undone.") at the user's request - swipe still just snaps the row back and shows the dialog; the delete itself only happens on Confirm. New shared pieces: TransactionRepository.observeSpentByCategoryForMonth (one grouped query for all of Home's cards), BudgetProgressBar/BudgetStatusLabel (core/designsystem, ready for Trends), ConfirmDialog (core/designsystem, pre-planned in the architecture doc but not built until this), and AppScaffold's `useBackArrow` (pushed screens - Add transaction, Category detail - now get a real back arrow instead of the hamburger that opened the side panel mid-flow, a small inconsistency this also fixed for Add transaction). Unit tests cover both ViewModels: per-card status/remaining, totals, review count, next-month-disabled-at-present, swipe-asks-first/cancel/confirm for both manual and SMS-linked transactions, month navigation |
| M8 | Built, not yet tested on-device (install succeeded; the phone disconnected before a live check) | 2026-09-25 | EvaluateBudgetAlerts (new domain use case): called after a transaction is saved - manual add or accepting a message, never edit or delete - compares spend to budget via the existing BudgetMaths, fires over-budget first and only checks 80% when it isn't crossed (so a jump straight from 79% to 101% fires only the over-budget alert), and is gated by AlertLogRepository so each (month, category, type) fires at most once regardless of how many times it's called afterward - confirmed by test that deleting and re-adding spend in the same month doesn't re-fire an already-fired alert. The alert-log record happens whether or not its switch is on, so turning a switch on mid-month never floods past-due alerts; only the notification itself is gated. New Notifier.showBudgetAlert with its own "Budget alerts" channel, kept behind a new BudgetAlertNotifier interface so EvaluateBudgetAlerts is unit-testable without a real Android Context (Notifier implements it). Settings page (replacing its placeholder): permission status for SMS/notifications/battery-unrestricted with direct fix actions (including the system "ignore battery optimizations" dialog, not just a link to general settings) refreshed on resume, and the three alert switches backed by new SettingsRepository/DataStoreSettingsRepository (SettingsDataStore itself gained the 3 boolean prefs, matching what its own doc comment already said would land here). Unit tests: the M8 alert-case table (79%→81%, 79%→101% jump, repeated crossing, ₹0 budget, delete-then-readd, switch off still records but doesn't notify) plus AddTransactionViewModel tests confirming a save fires an alert and an edit never does |
...
**Build:** Home with `SummaryCard`, `CategoryCard`, `BudgetProgressBar`, the review line (Not assigned count), month selector (past months viewable, future disabled), and the Add button. The Category detail page with `TransactionRow`, edit through M4's screen, and swipe-to-delete with a `ConfirmDialog` (revised 2026-09-25 from an initial Undo-bar build - deleting a transaction felt worse to make silently-reversible than to just ask first), which returns the linked message to Not assigned.
...
Manifest: `RECEIVE_SMS`, `READ_SMS`, `POST_NOTIFICATIONS`, `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` (added in M8 - not a runtime-prompted permission, just lets Settings show the system "ignore battery optimizations" dialog directly), the `SmsReceiver` entry (as in the Phase 1 test app), `allowBackup="false"`, and **no** INTERNET permission.
