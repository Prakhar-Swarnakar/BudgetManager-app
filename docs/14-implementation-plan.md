...
**Status on 25 September 2026:** Phase 1 run and mostly confirmed. M0 to M3 done. M5 (Categories) done and confirmed on-device. M4 (Add transaction and the accept flow) built and mostly confirmed on-device; one item (the Cancel button) has a fix shipped but not yet confirmed - see the progress tracker and the "still open" list in [07-open-questions.md](07-open-questions.md). M6 (Monthly budget) built, not yet tested on-device. M7 (Home and Category detail) is next.
...
| M5 | Done, confirmed on-device 2026-09-25 | 2026-09-25 | Categories page: a plain-Column list (drag handle to reorder, swipe left to archive), a collapsed Archived section that expands and restores on tap, and a bottom sheet shared between "new category" and "rename/change icon" (EmojiIconBox + name). The + button lives in the top bar via a new `actions` slot on AppScaffold. Reordering is local-only until drag end, then persisted in one batch write (CategoryRepository.reorder, CategoryDao.updateAll). Single-emoji validation (EmojiValidation, pure Kotlin, grapheme-cluster based) has tests for skin tones, ZWJ families, and flags. Fixed two real on-device bugs from first testing: dragging got stuck mid-gesture (the per-row list had no `key()`, so Compose matched rows by screen position rather than by category - a mid-drag reorder could cancel the drag's own gesture coroutine; fixed by keying each row on its id) and not every category could be archived (the list had no scroll modifier, so rows past the 9 starter categories were off-screen and unreachable; fixed by scoping `verticalScroll` to the populated-list branch only, since `EmptyState`'s `fillMaxSize()` would crash under an unbounded scrollable parent). Retested and confirmed working on all points |
| M6 | Built, not yet tested on-device | 2026-09-25 | Monthly budget page: MonthSelector (previous/next, no future restriction here - unlike Home's planned one, setting next month's budget ahead of time is normal), a row per active category (muted ₹0 when unset), a total at the bottom, and a bottom sheet shared between "edit this category's amount" (tap a row) and "new category with its own budget" (the + button, reusing the `actions` slot on AppScaffold that M5 added). The + never touches an existing category's amount. New domain use case CopyBudgetFromPreviousMonth: a month with no budget rows yet copies the previous month's in as a starting point (self-gating - a month with any rows, copied or user-edited, won't copy again), with a "Copied from last month" note shown when it happens. Unit tests cover the use case directly and the ViewModel (zero rows, editing, the + flow, totals, copy-forward, no-copy-when-nothing-to-copy). Installed on-device but not yet exercised by hand |
| M7 to M12 | Not started | | |
...
**Build:** the Categories page with drag to reorder, swipe to archive, and an archived section; the New category sheet with `EmojiIconBox` and a name, accepting a single emoji only; rename and change icon.

**Tests:** single-emoji validation, order persistence, and that archived categories drop out of pickers and suggestions but keep history.

**Done when:** you can create, reorder, rename, and archive categories on the phone, using the phone's emoji keyboard.

### M6. Monthly budget

**Build:** the Monthly budget page with `MonthSelector` and the total bar; `FormSheet` used both to edit an amount (tap a row) and to create a category with its amount (the + button); `CopyBudgetFromPreviousMonth`; the first-launch flow (starter categories at ₹0, then this page).

**Tests:** copy-forward (including when the previous month has no budget), totals, and the ViewModel.

**Done when:** every category is listed with ₹0 where nothing is set, tapping edits, + creates a new category and never changes an existing amount, and a new month starts as a copy.
...
4. Drag-to-reorder on Categories (M5). Categories then keep creation order.
